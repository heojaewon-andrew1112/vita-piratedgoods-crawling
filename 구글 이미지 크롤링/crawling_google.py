...
# 이미지 다운로드 함수
def image_download(keyword, folder_name, file_name): # 검색할 키워드, 생성할 폴더 이름, 파일 이름을 넘겨받는다.
        start = time.time()
        create_folder('C:/vita_piratedgoods_crawling/구글 이미지 크롤링/data/google_image_download/' + str(folder_name)) # 원하는 경로에 createFolder 함수로 폴더 생성
        ...
        # 키워드 검색 창에 입력
        keyword_input = driver.find_element(By.XPATH, '//*[@id="APjFqb"]')
        ...
        try:
            driver.find_element(By.XPATH, '//*[@id="islmp"]/div/div/div[2]/div[1]/div[2]/div[2]/input').send_keys(Keys.ENTER)# 결과 더보기 버튼 클릭
            for i in range(60): # 더보기로 펼친다음 다시 스크롤
                elem.send_keys(Keys.PAGE_DOWN)
                time.sleep(0.1)
        ...
        for img in images:
            i+=1
            if(index < 80): # images 중에 80개 이미지만 수집한다
                try:
                    webdriver.ActionChains(driver).click(img).perform() # 개별 이미지 클릭
                    time.sleep(0.5)
                    contains_any = False

                    img_url = driver.find_element(By.XPATH, '//*[@id="Sva75c"]/div[2]/div/div[2]/div[2]/div[2]/c-wiz/div/div/div/div[3]/div[1]/a/img[1]' 
).get_attribute("src") # 클릭 후, 원본 이미지 src 주소 추출
                ...
                try:
                    with urlopen(img_url) as f:
                        with open('C:/vita_piratedgoods_crawling/구글 이미지 크롤링/data/google_image_download/' + str(folder_name) + '/' + str(file_name) + str(index) + '.jpg', 'wb') as h: # 폴더 열고, 이미지 다운로드
                            r_img = f.read()
                            h.write(r_img)
                            index += 1 # 다운로드까지 모두 완료되었을때 인덱스 올림
                    print(f'{keyword} 이미지 다운로드 완료: {index} / {len(images)}\n')
                except Exception as e:
                    print(f'{i}번째 이미지 다운로드 실패.\n')
        ...
        tot_time = end - start
        print("시간 : " + str(tot_time) + ", " + str(keyword) + ' ---다운로드 완료---') # 이미지 다운로드에 걸린 시간
        print("")

        driver.close() # 품목 하나 다운받은 후 driver 닫기

# 엑셀 파일에서 키워드, 폴더명, 파일명 추출
meta_file = pd.read_excel('google_data.xlsx')
folder_name = []
keyword = []
file_name = []
